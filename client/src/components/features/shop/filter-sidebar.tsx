    ...
    <aside className="w-full bg-white px-4 lg:px-6 py-6 flex flex-col h-full text-left select-none">
      ...
      <div className="flex-1 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden divide-y divide-gray-100">
        ...
        <div className="py-3.5">
          ...
          >
            <span className="text-[13px] font-semibold uppercase tracking-wider text-gray-900">
              Category
            </span>
              ...
              {parentCategories.map((cat) => {
                ...
                return (
                  <div key={cat.id || cat.slug} className="space-y-0.5">
                    ...
                    >
                      <span className="truncate pr-2 text-[12px]">{cat.name}</span>
                      <span
                        className={`text-[11px] px-1.5 py-0.5 rounded font-mono ${isSelected ? 'bg-white/20 text-white' : 'text-gray-400 bg-gray-100'
                          }`}
                      >
                        {count}
                      </span>
                    ...
                    {isSelected && childSubs.length > 0 && (
                      <div className="ml-2 pl-2.5 border-l-2 border-gray-200 py-1 space-y-1">
                        <button
                          ...
                          className={`w-full flex items-center justify-between py-1 px-1.5 rounded text-[12px] transition-colors cursor-pointer ${!subCategory && (category?.toLowerCase() === cat.slug.toLowerCase() || category?.toLowerCase() === cat.name.toLowerCase())
                              ? 'bg-gray-100 font-medium text-black'
                        ...
                        {childSubs.map((sub) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between py-1 px-1.5 rounded text-[12px] transition-colors cursor-pointer ${isSubSelected
                                  ? 'bg-black text-white font-medium'
                                  : 'text-gray-600 hover:text-black hover:bg-gray-50'
                                }`}
                            >
                              ...
                              <span
                                className={`text-[11px] px-1.5 py-0.5 rounded font-mono ${isSubSelected ? 'bg-white/20 text-white' : 'text-gray-400 bg-gray-100'
                                  }`}
                              >
                                {subCount}
                              </span>
                ...
                {pricePresets.map((preset) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left text-[12px] py-1 px-2 rounded transition-colors cursor-pointer ${isCurrent
              ...
              <form onSubmit={handleApplyPriceInputs} className="pt-2 border-t border-gray-100">
                <div className="flex items-center gap-1.5">
                  <div className="relative flex-1">
                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] text-gray-600 font-semibold select-none">₹</span>
                    <input
                      type="number"
                      placeholder="Min"
                      value={inputMin}
                      onChange={(e) => setInputMin(e.target.value)}
                      className="w-full pl-5 pr-1.5 py-1 text-[12px] text-gray-900 bg-white placeholder:text-gray-400 border border-gray-300 rounded focus:outline-none focus:border-black focus:ring-1 focus:ring-black font-medium"
                    />
                  </div>
                  <span className="text-gray-500 text-[12px] font-medium">-</span>
                  <div className="relative flex-1">
                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] text-gray-600 font-semibold select-none">₹</span>
                    <input
                      type="number"
                  ...
                  <button
                    type="submit"
                    className="px-2.5 py-1 bg-black text-white text-[11px] font-medium uppercase tracking-wider rounded hover:bg-gray-800 transition-colors cursor-pointer"
                  >
                    Go
                  </button>
                </div>
                ...
                (sizeItem: any) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative py-2 px-1 text-[12px] font-medium rounded border transition-all cursor-pointer ${isSelected
                    ...
                    >
                      ...
                      {sizeItem.count > 0 && (
                        <span
                          className={`block text-[11px] font-normal leading-none mt-0.5 ${isSelected ? 'text-gray-300' : 'text-gray-400'
                            }`}
                        >
                          {sizeItem.count}
                        </span>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] text-gray-800">{fitVal}</span>
                      </div>
                      {fitItem.count > 0 && (
                ...
                (colorItem: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-[12px] font-normal text-gray-800 capitalize">
                        {colorName}
                      </span>
                    </label>
                  );
            ...
            <div className="mt-3 space-y-2 text-[12px]">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] text-gray-800 font-medium">In Stock Only</span>
                </div>
              </label>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] text-gray-800 font-medium flex items-center gap-1">
